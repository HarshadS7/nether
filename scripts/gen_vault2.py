"""Generate tempVault2 from demo_output_fastapi.json"""
import json, re
from pathlib import Path
from collections import defaultdict

INPUT = Path(__file__).parent / "demo_output_fastapi.json"
VAULT = Path(__file__).parent / "tempVault2"
...
print(f"Loaded {len(files)} files from {INPUT.name}")
...
print(f"Definitions (non-dunder): {len(all_defs)}")
...
# ── Definition notes ─────────────────────────────────
print("Generating definition notes...")
for d, f in all_defs:
...
idx.append("\n## Key Definitions (top 50)\n")
for d, f in all_defs[:50]:
    e = "🔷" if d['type'] == 'class' else "🔹"
    idx.append(f"- {e} [[{def_note_name(d, f)}]]")
if len(all_defs) > 50:
    idx.append(f"\n*...and {len(all_defs) - 50} more definitions*")
